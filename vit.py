...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,prompt=False):
        super().__init__()
...
class Block(nn.Module):
    ...
    def __init__(
            ...
            drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm,use_prompt=None,layer=None, args=None):
        super().__init__()
        self.norm1 = norm_layer(dim)
        prompt=False
        if use_prompt is not None:
            if layer in use_prompt:
                prompt=True
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop,prompt=prompt)
        ...
        if args.use_adapter:
            print('init_adapter')
            self.adaptmlp = Adapter(args, dropout=0.1, bottleneck=args.ffn_num,
    ...
    def forward(self, x):
        ...
        if self.args.use_adapter and self.args.ffn_option == 'parallel':
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            class_token=True, use_prompt=[],no_embed_class=False, fc_norm=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,
            weight_init='', embed_layer=PatchEmbed, norm_layer=None, act_layer=None, block_fn=Block, args=None):
        """
        Args:
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer,use_prompt=use_prompt,layer=i,args=args)
...
def vit_base_patch16_224(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, num_heads=12, **kwargs)
